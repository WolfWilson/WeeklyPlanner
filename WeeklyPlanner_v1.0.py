...
class NotificationDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Selector de tiempo para la notificación
        self.time_edit = QTimeEdit(self)
        self.time_edit.setDisplayFormat('HH:mm')
        self.time_edit.setTime(QTime.currentTime())
        self.layout.addWidget(self.time_edit)
        
        # Selector de repetición de notificaciones
        self.repeat_combo = QComboBox(self)
        self.repeat_combo.addItem("No repetir")
        self.repeat_combo.addItem("Repetir diariamente")
        self.repeat_combo.addItem("Repetir semanalmente")
    ...
    def get_repetition(self):
        return self.repeat_combo.currentText()
...
class WeeklyPlanner(QMainWindow):
    ...
    def configure_notification(self, position):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            repetition = dialog.get_repetition()
            row = self.table.indexAt(position).row()
            column = self.table.indexAt(position).column()
            widget = self.table.cellWidget(row, column)
            if widget:
                label_text = f"{widget.findChild(QLabel).text()} (Notif. {notification_time})"
                widget.findChild(QLabel).setText(label_text)
                self.notifications[(row, column)] = (notification_time, repetition)
                logging.info(f'Notificación configurada para fila {row}, columna {column} a las {notification_time}, repetición: {repetition}')
                self.save_to_excel()
    ...
    def remove_notification(self, position):
        ...
        if widget:
            ...
            if (row, column) in self.notifications:
                del self.notifications[(row, column)]
        ...
        for row in range(self.table.rowCount()):
            for column in range(self.table.columnCount()):
                ...
                if widget:
                    ...
                    if (row, column) in self.notifications:
                        notification_time, repetition = self.notifications[(row, column)]
                        if notification_time == now and not checkbox.isChecked():
                            self.show_popup_notification(text, row, column)
                            if repetition == "Repetir diariamente":
                                next_notification = datetime.now() + timedelta(days=1)
                            elif repetition == "Repetir semanalmente":
                                next_notification = datetime.now() + timedelta(weeks=1)
                            else:
                                next_notification = None

                            if next_notification:
                                new_time = next_notification.strftime('%H:%M')
                                self.notifications[(row, column)] = (new_time, repetition)
    ...
    def show_popup_notification(self, text, row, column):
        ...
        # Centrar la ventana de notificación
        self.center_message_box("Recordatorio de Actividad", f'Tienes una actividad programada: {notification_text} para {day}')
        logging.info(f'Notificación emergente mostrada para actividad: {notification_text} en {day}')
    
    def center_message_box(self, title, message):
        msg_box = QMessageBox(self)
        msg_box.setWindowTitle(title)
        
        # Ajustar el texto con HTML para mejorar el formato
        formatted_message = f"<p style='font-size: 14px; text-align: center;'>{message}</p>"
        
        msg_box.setText(formatted_message)
        msg_box.setIcon(QMessageBox.Icon.Information)
        
        # Asegurarnos de que el tamaño mínimo sea suficiente para mostrar el texto completo
        msg_box.setStyleSheet("""
            QLabel{min-width: 300px; min-height: 100px; font-size: 14px;}
            QPushButton{min-width: 80px; background-color: #4CAF50; color: white;}
        """)
        
        # Centrar la ventana de notificación en la pantalla
        screen_geometry = QApplication.primaryScreen().geometry()
        msg_box.move(screen_geometry.center() - msg_box.rect().center())
        
        msg_box.exec()
        pygame.mixer.music.stop()  # Detener el sonido al cerrar la notificación
